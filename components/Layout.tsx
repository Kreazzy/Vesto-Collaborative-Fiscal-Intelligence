    ...
    <div className="relative flex flex-col min-h-screen bg-background">
      ...
      <header className="hidden md:flex items-center justify-between px-10 py-6 border-b border-white/5 bg-surface/30 backdrop-blur-xl sticky top-0 z-50">
        ...
        <nav className="flex items-center gap-8">
          ...
          <button 
            onClick={logout}
            className="flex items-center gap-2 text-red-500 hover:text-red-400 font-bold ml-4"
          >
      ...
      <nav className="md:hidden fixed bottom-0 left-0 right-0 z-50">
        ...
        <div className="bg-[#152019]/90 backdrop-blur-xl border-t border-white/5 px-6 pt-4 pb-8 flex justify-between items-center">
          {navItems.map((item) => (
            <button
              key={item.id}
              onClick={() => setActiveTab(item.id)}
      ...
      <footer className="hidden md:flex py-8 justify-center opacity-30">
        <a href="https://cordulatech.com" target="_blank" className="text-xs font-bold tracking-widest uppercase hover:text-primary">
